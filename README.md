...
- **Project folders**: sanitized, human-readable slugs generated by `generateUniqueFolderName()` at creation time (e.g. `Sunset Shoot`, `Sunset Shoot (2)`). Legacy `p<id>` folders remain supported during the transition and are realigned by maintenance when necessary. See `server/utils/projects.js` for slug helpers and legacy compatibility utilities.
...
- **Projects**
  ...
  - `PATCH /api/projects/:folder/rename` — rename (display name only; maintenance will align folder automatically)
  - `DELETE /api/projects/:folder` — archive and queue deletion task (folder uses sanitized slug or legacy `p<id>`)
- **Project details**
...
- **Real-time Updates**: Live progress tracking for all background tasks
  ...
  - The Processes panel now always reads from the tenant-wide `GET /api/jobs` endpoint, so admins see every in-flight job from All Photos or any project without switching context.
