...
const ProjectSelector = ({ projects, selectedProject, onProjectSelect, disabled = false, placeholderLabel }) => {
      ...
      <div className="relative">
        <select
          ...
          className={`block w-32 sm:w-64 pl-2 pr-6 sm:pl-3 sm:pr-10 py-1.5 sm:py-2 border border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 text-xs sm:text-sm rounded-md truncate ${
            (projects.length === 0 || disabled) ? 'bg-gray-100 text-gray-500 cursor-not-allowed' : ''
          }`}
        >
          {(projects.length === 0) ? (
            ...
            <>
            {disabled && (
              <option value="">{placeholderLabel || 'All Projects'}</option>
            )}
