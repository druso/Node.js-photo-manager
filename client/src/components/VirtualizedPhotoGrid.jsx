...
const VirtualizedPhotoGrid = ({
  ...
  useEffect(() => {
    if (photos && photos.length > 0) {
      ...
      const key = `${first.project_folder || projectFolder || 'pf'}::${first.filename}`;
      console.log('[VirtualizedPhotoGrid] First photo key:', key, 'Selected count:', selectedPhotos?.size || selectedPhotos?.length);
      if (selectedPhotos && typeof selectedPhotos.has === 'function') {
        console.log('[VirtualizedPhotoGrid] Is first photo selected?', selectedPhotos.has(key));
      }
    }
  }, [photos, selectedPhotos, projectFolder]);
          ...
          {containerWidth > 0 && (
            <div className="relative">
              ...
              {hasMore && (
                <div className="flex justify-center py-4">
                  <button
                    disabled={paginationStatus === 'loading_more'}
                    onClick={() => {
                    ...
                    className={`px-6 py-2 rounded-lg text-white transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 ${paginationStatus === 'loading_more'
                        ? 'bg-blue-400 cursor-wait'
                        : 'bg-blue-600 hover:bg-blue-700'
                      }`}
                  >
                    {paginationStatus === 'loading_more' ? (
