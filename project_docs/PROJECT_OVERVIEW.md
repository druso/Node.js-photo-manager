...
Projects are stored on disk under `<repoRoot>/.projects/user_0/<project_folder>/` where `project_folder` is always of the form `p<id>` (immutable and decoupled from the display name). The `user_0` folder provides user-scoped isolation and enables future multi-user support. Duplicate human names are allowed; uniqueness is enforced by `project_folder`. The numeric `id` is returned by all project APIs and should be used for renames; the folder never changes.
...
**Folder Alignment** (Automatic via Maintenance)
...
- **Run**: `npm test`
- **Coverage**: 50/53 tests passing (94%)
...
**Test Cleanup**:
- Each test suite tracks created resources (`projectIds`, `projectFolders`, `linkIds`)
- `cleanupTestData()` removes database entries and filesystem folders
- Retry logic (5 attempts with exponential backoff) handles SQLITE_BUSY errors
- WAL checkpoint before cleanup: `db.pragma('wal_checkpoint(TRUNCATE)')`
- 100ms delay between tests via `afterEach()` hook
- Final cleanup via `after()` hook

**Test Data**:
- Projects created with unique names: `Test Project ${Date.now() + Math.random()}`
- Folders placed in `.projects/user_0/` (user-scoped)
- All test folders automatically removed after tests complete
