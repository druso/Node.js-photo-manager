...
const { runTrashMaintenance, runDuplicateResolution, runManifestCheck, runFolderCheck, runManifestCleaning, runFolderAlignment, runOrphanedProjectCleanup, runDerivativeCacheValidation, runWebPMigration } = require('./workers/maintenanceWorker');
...
async function handleJob(job, { heartbeatMs, maxAttemptsDefault, workerId }) {
  ...
  try {
    ...
    if (job.type === 'webp_migration') {
      await runWebPMigration(job);
      stopHeartbeat();
      jobsRepo.complete(job.id);
      {
        const p = job.payload_json || {};
