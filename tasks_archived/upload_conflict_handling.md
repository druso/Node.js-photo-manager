...
The frontend upload modal is not honoring the conflict information returned by the analyze endpoint, causing a mismatch between client intent and server validation.

- Endpoint: `POST http://localhost:3000/api/projects/:folder/analyze-files` (e.g., `p2`).
- Issue: The modal proceeds to allow uploads even when the analyze response includes cross-project conflicts (e.g., `conflicts` entries). As a result, the server correctly blocks those uploads on `POST /api/projects/:folder/upload` when it detects duplicates in other projects.
- Impact: Users see failed uploads for conflicted items instead of being guided to reload/move them or exclude them from the upload set based on the analysis.
- Expected behavior: The client should respect the analysis outcome by:
  - Excluding cross-project conflicts from the upload unless the user explicitly opts to reload/move them into the current project.
  - Clearly surfacing the conflicts section in the confirmation step and wiring the chosen option into the upload request.
