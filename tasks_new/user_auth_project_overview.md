...
This will open a view similar to a Projecy view, but will show all photos assigned to that shared link. 
...
## Milestone 0 Authentication Baseline (2025-10-04)

- **Environment contract**: `AUTH_ADMIN_BCRYPT_HASH`, `AUTH_JWT_SECRET_ACCESS`, `AUTH_JWT_SECRET_REFRESH` now required; optional `AUTH_BCRYPT_COST` (8–14, default 12) controls bcrypt work factor.
- **Startup validation**: `server/services/auth/initAuth.js` invokes `ensureAuthConfig()` during boot and terminates on misconfiguration; logs include `auth_config_invalid` with context.
- **Helper modules**: `passwordUtils.js`, `tokenService.js`, and `authCookieService.js` provide password verification, JWT issuance/verification (1 h access / 7 d refresh, issuer `photo-manager`, audience `photo-manager-admin`), and HTTP-only cookie management.
- **Tests**: Node `node:test` suites under `server/services/auth/__tests__/` cover config successes/failures, password helpers, token lifecycle, and cookie behaviour (`npm test`).
- **Documentation**: `.env.example`, `README.md`, `PROJECT_OVERVIEW.md`, `SCHEMA_DOCUMENTATION.md`, `SECURITY.md` updated with auth guidance and migration scaffolding notes.
- **Migrations**: Draft scripts and runner scaffolding exist (`server/services/migrations/`) for `photos.visibility`, `public_links`, and `photo_public_links` to support future public-sharing workflows.